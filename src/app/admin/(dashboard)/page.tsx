...
import { getAdminStats, getPendingVerifications, getProviderGrowth, getCityDistribution } from '@/lib/admin-queries';
...
export default async function AdminPage() {
  const [stats, pendingVerifs, growth, cityDist] = await Promise.all([
    getAdminStats(),
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
        <div className="bg-white border border-zinc-200 rounded-lg p-6">
          <h2 className="font-semibold mb-4">服务者增长趋势</h2>
          <LineChart data={growth.map(g => ({ label: g.month, value: g.total }))} />
        </div>
        <div className="bg-white border border-zinc-200 rounded-lg p-6">
          <h2 className="font-semibold mb-4">城市分布（Top 10）</h2>
          <BarChart data={cityDist.map(c => ({ label: c.cityName, value: c.count }))} />
        </div>
      </div>
